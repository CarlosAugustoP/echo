    ...
    public class SendToVendorConfirmedNotification : NotificationBase<SendToVendorConfirmedNotificationModel>
    {
        ...
        protected override List<Notification> CreateCore(SendToVendorConfirmedNotificationModel model)
        {
            ...
            string donorDescription =
                $"Sua doacao de {model.Amount.ToString("G29")} ETH para o projeto {model.ProjectName} foi confirmada na blockchain e enviada ao fornecedor {model.VendorName} para atender a meta {model.GoalTitle}.";

            const string ngoMessage = "Transferencia para o fornecedor confirmada!";
            string ngoDescription =
                $"A doacao de {model.Amount.ToString("G29")} ETH do projeto {model.ProjectName} foi confirmada na blockchain e recebida pelo fornecedor {model.VendorName} para a meta {model.GoalTitle}.";
